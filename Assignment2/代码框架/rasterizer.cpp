...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    float boundL = min(v[0].x(),min(v[1].x(),v[2].x()));
    float boundR = max(v[0].x(),max(v[1].x(),v[2].x()));
    float boundB = min(v[0].y(),min(v[1].y(),v[2].y()));
    float boundT = max(v[0].y(),max(v[1].y(),v[2].y()));
